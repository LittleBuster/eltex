...
int main(void)
{
	...
	// Test description
	ret_val = strcmp(info->description, "Substraction of numbers.");
	if (ret_val != 0) {
	...
	// Test func name
	ret_val = strcmp(info->func_name, "sub");
	if (ret_val != 0) {
	...
	ret_val = sub(10, 3);
	if (ret_val != 7) {
		puts("[ERROR] Test substraction function.");
		exit_test(false);
		assert(ret_val == 7);
	}
	puts("[PASSED] Test substraction function.");
